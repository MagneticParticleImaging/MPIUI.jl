...
function updateProtocol(pw::ProtocolWidget, protocol::Protocol)
  ...
  @idle_add_guarded begin
    ...
    # we need to update the study here, the data handler does not get initialized properly unless the study selection in the main widget changes
    if !isnothing(mpilab[].currentStudy)
